...
tester = Account.objects.get(user=User.objects.get(username='tester'))
...
for i in range(10):
    Note.objects.create(title='Go To Land {0}'.format(i),
                        owner=tester)

for i in range(10):
    Reminder.objects.create(title='Go to lib {0} next thursday'.format(i),
                            owner=tester,
                            alarm_time=datetime.datetime(2012,6,3,10,2))
